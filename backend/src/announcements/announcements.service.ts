...
export class AnnouncementsService {
  ...
  async list(userId: string, groupId?: string) {
    const membershipFilter = {
      members: {
        some: { userId, status: 'APPROVED' as const },
      },
    };

    const list = await this.prisma.announcement.findMany({
      where: groupId
        ? { groupId, group: membershipFilter }
        : {
            OR: [{ groupId: null }, { group: membershipFilter }],
          },
      include: {
        author: {
          select: {
  ...
  async create(userId: string, dto: CreateAnnouncementDto) {
    ...
    if (dto.groupId) {
      ...
      if (!membership || membership.status !== 'APPROVED') {
        throw new ForbiddenException(
          '모임 회원만 게시글을 작성할 수 있습니다.',
        );
      }
    } else {
      if (!user || user.role !== 'ADMIN') {
        throw new ForbiddenException(
          '운영자만 전체 공지사항을 작성할 수 있습니다.',
        );
      }
    }
