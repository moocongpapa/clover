...
export function isProfileComplete(user?: User | null): boolean {
  ...
  const hasPhone = Boolean(
    user.phoneNumber && user.phoneNumber.trim().length >= 8,
  );
  return hasName && hasBirth && hasGender && hasPhone;
}

export async function compressFileToBase64DataUrl(
  file: File,
  maxPx = 600,
  quality = 0.82,
): Promise<string> {
...
export async function startKakaoLogin() {
  ...
  if (!restApiKey) {
    throw new Error("카카오 로그인 설정을 불러오지 못했습니다.");
  }
...
export const api = {
  ...
  listAnnouncements: (groupId?: string) =>
    request<Announcement[]>(
      groupId ? `/announcements?groupId=${groupId}` : "/announcements",
    ),
  ...
  listMyAnnouncements: () => request<Announcement[]>("/announcements/mine"),
  ...
  getAttendanceStats: (groupId: string) =>
    ...
    }>(`/groups/${groupId}/attendance-stats`),
  ...
  uploadGalleryFile: async (
    ...
    // Gallery/chat media always passes through the API so the server can verify
    // its binary signature and assign a safe extension before it is displayed.
    const token = getToken();
  ...
  deleteAllNotifications: async (): Promise<{
    ...
    try {
      ...
      try {
        return await request<{ ok: boolean }>("/notifications/all", {
          method: "DELETE",
        });
      } catch {
        return await request<{ ok: boolean }>("/notifications/delete-all", {
          method: "POST",
        });
      }
    }
  ...
  togglePayment: (
    ...
    request<any>(
      `/groups/${groupId}/payments/${userId}/toggle?year=${year}&month=${month}`,
      {
        method: "POST",
      },
    ),
  ...
  remindUnpaidMembers: (groupId: string, year: number, month: number) =>
    request<{ remindedCount: number }>(
      `/groups/${groupId}/payments/remind?year=${year}&month=${month}`,
      {
        method: "POST",
      },
    ),
  ...
  createGroupMedia: (
    groupId: string,
    data: { url: string; fileType: string },
  ) =>
    request<any>(`/groups/${groupId}/media`, {
      method: "POST",
      body: JSON.stringify(data),
  ...
  getActiveSystemAnnouncements: () =>
    request<SystemAnnouncementItem[]>("/public/system-announcements"),
  ...
  admin: {
    ...
    updateUser: (
      id: string,
      data: { role?: string; isBlocked?: boolean; displayName?: string },
    ) =>
    ...
    getGroups: (params?: {
      ...
      if (params?.isPublic !== undefined)
        q.set("isPublic", String(params.isPublic));
      if (params?.page) q.set("page", String(params.page));
      if (params?.limit) q.set("limit", String(params.limit));
    ...
    updateGroup: (
      id: string,
      data: { isPublic?: boolean; category?: string; maxMembers?: number },
    ) =>
    ...
    deleteGroup: (id: string) =>
      request<{ ok: boolean; message?: string }>(`/admin/groups/${id}`, {
        method: "DELETE",
      }),
    ...
    updateRole: (
      key: string,
      data: { label?: string; isStaff?: boolean; sortOrder?: number },
    ) =>
    ...
    deleteAnnouncement: (id: string) =>
      request<{ ok: boolean }>(`/admin/announcements/${id}`, {
        method: "DELETE",
      }),
...
export function parseNotificationMessage(rawMessage: string): {
  title: string;
  detail: string | null;
} {
...
export function getShortBirthYear(
  birthYear?: number | null,
  birthDate?: string | null,
): string {
  if (birthYear) {
...
export function normalizeCategory(category?: string | null): string {
  ...
  if (
    ...
    return "탁구";
  if (lower.includes("배드민턴") || lower.includes("badminton"))
    return "배드민턴";
...
export function formatCategoryEmoji(category: string): string {
  ...
  return CATEGORY_OPTIONS.find((c) => c.value === normalized)?.emoji ?? "✨";
}
