...
export default function Login() {
  ...
  const handleKakaoStart = async (e: React.MouseEvent) => {
    ...
    setError(
      "카카오 로그인 설정을 불러오지 못했습니다. 잠시 후 다시 시도해주세요.",
    );
  };
  ...
  return (
    <div className="login-page">
      ...
      {showKakaoModal && (
        <div
          className="kakao-modal-backdrop"
          onClick={() => setShowKakaoModal(false)}
        >
          <div
            className="kakao-modal-card"
          ...
          >
            ...
            <form
              onSubmit={handleKakaoModalSubmit}
              className="kakao-modal-body"
            >
              <h2 className="kakao-modal-title">카카오계정 로그인</h2>
              <p className="kakao-modal-sub">
                Clover 서비스 이용을 위해 카카오 계정으로 로그인해주세요.
              </p>

              <div className="kakao-modal-field">
                <label className="kakao-modal-label">
                  카카오계정 (이메일 또는 닉네임)
                </label>
                <input
                  type="text"
              ...
              <div className="kakao-modal-tip">
                💡 <b>안내</b>: <code>backend/.env</code> 파일의{" "}
                <code>KAKAO_REST_API_KEY</code>에 Kakao Developers REST API 키를
                발급받아 입력하시면 카카오 공식 OAuth2 로그인 서버로 자동
                전환됩니다.
              </div>
            </form>
          </div>
