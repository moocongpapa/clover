    ...
    <div className="event-detail-page">
                        ...
                        {voters.map((v) => (
                          <Link to={`/profile/${v.user.id}`} key={v.user.id} className="vote-voter-chip">
                            {v.user.profileImageUrl ? (
                              <img loading="lazy" decoding="async" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} src={v.user.profileImageUrl} alt="" className="vote-voter-chip__avatar" />
                            ) : (
            ...
            {votes.nonVoters.map((user) => (
              <Link to={`/profile/${user.id}`} key={user.id} className="vote-voter-chip vote-voter-chip--nonvoter">
                {user.profileImageUrl ? (
                  <img loading="lazy" decoding="async" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} src={user.profileImageUrl} alt="" className="vote-voter-chip__avatar" />
                ) : (
              ...
              <div key={comment.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '12px', background: 'var(--grey-50)', borderRadius: '12px' }}>
                ...
                <div style={{ flexShrink: 0 }}>
                  {comment.user.profileImageUrl ? (
                    <img loading="lazy" decoding="async" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} src={comment.user.profileImageUrl} alt="" style={{ width: '32px', height: '32px', borderRadius: '50%', objectFit: 'cover' }} />
                  ) : (
      ...
      <EventShareModal
        ...
        event={event ? {
          ...
          groupName: group?.name || event.group?.name,
          groupProfileImageUrl: group?.profileImageUrl || event.group?.profileImageUrl,
